...
export default function HowItWorks() {
  const steps = [
    {
      icon: <FaCar className="w-12 h-12 text-orange-500 dark:text-orange-400" />,
    ...
    {
      icon: <FaCreditCard className="w-12 h-12 text-gray-700 dark:text-gray-400" />,
      title: "Buchen und Bezahlen",
      description:
        "Wählen Sie Ihre Mietdaten, überprüfen Sie die Details und zahlen Sie sicher online.",
    },
    {
      icon: <FaKey className="w-12 h-12 text-orange-500 dark:text-orange-400" />,
      title: "Abholung",
      description:
        "Holen Sie Ihr Fahrzeug am vereinbarten Ort ab. Wir stellen sicher, dass alles bereit ist.",
    },
    {
      icon: <FaCheckCircle className="w-12 h-12 text-green-500 dark:text-green-400" />,
        ...
        <div className="container mx-auto py-12 px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {steps.map((step, index) => (
              <div
                key={index}
                className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-elegant hover:shadow-elegant-lg transition-smooth card-hover"
              >
                <div className="flex flex-col items-center text-center">
                  <div>
                    {step.icon}
                  </div>
                  <h3 className="text-xl font-semibold mt-4 mb-3 text-gray-900 dark:text-white font-display">
                    {step.title}
                  </h3>
