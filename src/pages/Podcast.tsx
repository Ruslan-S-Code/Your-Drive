...
export default function Podcast() {
  ...
  // Отслеживание новых эпизодов подкаста
  useEffect(() => {
    const lastKnownEpisodes = localStorage.getItem('lastKnownPodcastEpisodes');
    const currentEpisodeIds = episodes.map(e => e.id).join(',');
    
    // Если это первый запуск, просто сохраняем текущие эпизоды без уведомлений
    if (!lastKnownEpisodes) {
      localStorage.setItem('lastKnownPodcastEpisodes', currentEpisodeIds);
      return;
    }
    
    // Если эпизоды изменились, проверяем новые
    if (lastKnownEpisodes !== currentEpisodeIds) {
      const lastIds = lastKnownEpisodes.split(',').map(Number);
      const newEpisodes = episodes.filter(e => !lastIds.includes(e.id));
      
      // Создаем уведомления только для новых эпизодов
      newEpisodes.forEach(episode => {
        addNotification({
          type: 'podcast',
          title: `Новый эпизод: ${episode.title}`,
          description: `${episode.duration} • ${episode.host}`,
          link: '/podcast',
        });
      });
      
      // Обновляем список известных эпизодов
      localStorage.setItem('lastKnownPodcastEpisodes', currentEpisodeIds);
    }
            ...
            <div
              ...
              className={`bg-white dark:bg-gray-800 rounded-lg shadow-sm overflow-hidden ${
                activeEpisode === episode.id
                  ? "ring-2 ring-gray-600 dark:ring-gray-500 dark:ring-gray-500 dark:ring-gray-400"
                  : ""
              }`}
            >
                ...
                <div className="p-6 md:w-2/3">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <button
                      ...
                      className={`p-3 rounded-full transition-smooth shadow-elegant hover:shadow-glow-orange button-press ${
                        activeEpisode === episode.id && isPlaying
                          ? "bg-orange-500 text-white"
                          : "bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-orange-500 hover:text-white dark:hover:bg-orange-500"
                      }`}
                    >
                      {activeEpisode === episode.id && isPlaying ? (
                  ...
                  <div className="flex flex-wrap gap-4 text-sm text-gray-500 dark:text-gray-400">
                    <div className="flex items-center">
                      <FaMicrophone className="mr-2 text-gray-600 dark:text-gray-400 dark:text-gray-400" />
          ...
          <div className="max-w-md mx-auto">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-600 dark:focus:ring-gray-500 dark:focus:ring-gray-500 dark:focus:ring-gray-400 bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
              />
              <button className="px-6 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-lg transition-smooth shadow-elegant hover:shadow-glow-orange button-press font-medium">
                Abonnieren
              </button>
            </div>
          </div>
